...
public class TreeSet<T> implements SortedSet<T> {
    ...
    private void displayNode(Node<T> node) {
        if (node != null) {
            System.out.printf("Node: %s, Parent: %s, Left: %s, Right: %s\n",
                    node.obj,
                    node.parent != null ? node.parent.obj : "null",
    ...
    private Node<T> invertTree(Node<T> root) {
        if(root == null){
            return root;
        }
        Node<T> left = invertTree(root.left);
        Node<T> right = invertTree(root.right);
        root.left = right;
        root.right = left;
        return root;
    }
